...
function tracesFor(rows, xMetric, yMetric, selectedInstitution) {
  ...
  if (selectedInstitution) {
    ...
    if (sr && Number.isFinite(sr[xMetric.key]) && Number.isFinite(sr[yMetric.key]) && sr[xMetric.key] > 0) {
      traces.push({
        name: "Selected",
        type: "scatter",
        mode: "markers",
...
function renderChart(divId, rows, xMetric, yMetric, selectedInstitution, onSelectInstitution, fitKind) {
  ...
  const xMaxData = x.length > 0 ? Math.max(...x) : 1;
  const xMaxWithBaseline =
    xMetric.key === "casual_share_actual_fte_2023_pct"
      ? Math.max(xMaxData, BASELINES.higherEdCasualShareFte2023Pct)
      : xMaxData;
  const xMax = Number.isFinite(xMaxWithBaseline) && xMaxWithBaseline > 0 ? xMaxWithBaseline : 1;
  const xMaxPad = xMax * 1.05;
...
async function main() {
  ...
  function formatFitStats(kind, fit, xMetric) {
    if (!fit) return "";
    const parts = [
      `slope ${fit.slope.toFixed(3)}` + (xMetric.unit === "percent" ? " per 1%" : ""),
      `r = ${fit.r.toFixed(3)}`,
      `R² = ${fit.r2.toFixed(3)}`,
      `n = ${fit.n}`,
    ];
    if (kind === "ransac" && typeof fit.inliers === "number") {
      parts.splice(3, 0, `inliers = ${fit.inliers}`);
    }
    return parts.join(" • ");
  }
