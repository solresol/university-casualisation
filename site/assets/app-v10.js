...
function logGamma(z) {
  // Lanczos approximation (g=7), good enough for the df in this dataset.
  const p = [
    0.99999999999980993,
    676.5203681218851,
    -1259.1392167224028,
...
function studentTPValueTwoSided(t, df) {
...
function olsFit(x, y) {
  ...
  if (df > 0) {
    const sigma2 = sse / df;
    const seSlope = Math.sqrt(sigma2 / varX);
    if (Number.isFinite(seSlope) && seSlope > 0) {
      const t = slope / seSlope;
      p = studentTPValueTwoSided(t, df);
    }
  }
  return { intercept, slope, r, r2, p, n, df };
}
...
async function main() {
  ...
  function formatFitStats(fit, xMetric) {
    if (!fit) return "";
    const per = xMetric.unit === "percent" ? " per 1%" : "";
    const p =
      fit.p === null || fit.p === undefined || !Number.isFinite(fit.p)
        ? "p = —"
        : fit.p < 0.001
          ? "p < 0.001"
          : `p = ${fit.p.toFixed(3)}`;

    const parts = [`slope ${fit.slope.toFixed(3)}${per}`, p, `R² = ${fit.r2.toFixed(3)}`];
  ...
  function renderAll() {
    ...
    qiltStats.textContent = formatFitStats(fitQ, qx);
    ...
    profitStats.textContent = formatFitStats(fitP, px);
  }
