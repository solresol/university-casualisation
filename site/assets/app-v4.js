...
const FIT_OPTIONS = [
  { key: "huber", label: "Huber (robust)" },
  { key: "theil_sen", label: "Theil–Sen (robust)" },
  { key: "ols", label: "OLS" },
  { key: "none", label: "None" },
];
...
function tracesFor(rows, xMetric, yMetric, selectedInstitution) {
  ...
  if (selectedInstitution) {
    ...
    if (sr && Number.isFinite(sr[xMetric.key]) && Number.isFinite(sr[yMetric.key])) {
      traces.push({
        name: "Selected",
        type: "scatter",
        mode: "markers",
...
function renderChart(divId, rows, xMetric, yMetric, selectedInstitution, onSelectInstitution, fitKind) {
  ...
  const clean = rows.filter((r) => Number.isFinite(r[xMetric.key]) && Number.isFinite(r[yMetric.key]));
  const x = clean.map((r) => r[xMetric.key]);
  const y = clean.map((r) => r[yMetric.key]);
  const fit = fitLine(fitKind, x, y);

  const traces = tracesFor(rows, xMetric, yMetric, selectedInstitution);

  if (fit) {
    const xMin = Math.min(...x);
    const xMax = Math.max(...x);
    const xGrid = [xMin, xMax];
    const yGrid = [fit.intercept + fit.slope * xMin, fit.intercept + fit.slope * xMax];
    traces.push({
  ...
  const layout = {
    ...
    showlegend: true,
    legend: { orientation: "h", y: -0.18, x: 0, font: { color: theme.text } },
...
async function main() {
  ...
  const qiltFit = byId("qilt-fit");
  const profitX = byId("profit-x");
  const profitY = byId("profit-y");
  const profitFit = byId("profit-fit");
  const qiltStats = byId("qilt-stats");
  const profitStats = byId("profit-stats");

  selectOptions(qiltX, X_METRICS, X_METRICS[0].key);
  selectOptions(qiltFit, FIT_OPTIONS, "huber");
  selectOptions(profitX, X_METRICS, X_METRICS[0].key);
  selectOptions(profitY, PROFIT_Y_METRICS, PROFIT_Y_METRICS[0].key);
  selectOptions(profitFit, FIT_OPTIONS, "huber");
  ...
  function renderAll() {
    const qx = X_METRICS.find((m) => m.key === qiltX.value) ?? X_METRICS[0];
    const px = X_METRICS.find((m) => m.key === profitX.value) ?? X_METRICS[0];
    const py = PROFIT_Y_METRICS.find((m) => m.key === profitY.value) ?? PROFIT_Y_METRICS[0];
    const qFit = qiltFit.value || "huber";
    const pFit = profitFit.value || "huber";

    const fitQ = renderChart("chart-qilt", rows, qx, QILT_Y, selectedInstitution, setSelectedInstitution, qFit);
    if (fitQ) {
      qiltStats.textContent = `${fitLabel(qFit)}: slope ${fitQ.slope.toFixed(3)} per 1% casual share • r = ${fitQ.r.toFixed(
        3
      )} • R² = ${fitQ.r2.toFixed(3)} • n = ${fitQ.n}`;
    } else {
      qiltStats.textContent = "";
    }

    const fitP = renderChart("chart-profit", rows, px, py, selectedInstitution, setSelectedInstitution, pFit);
    if (fitP) {
      profitStats.textContent = `${fitLabel(pFit)}: slope ${fitP.slope.toFixed(3)} • r = ${fitP.r.toFixed(3)} • R² = ${fitP.r2.toFixed(
        3
      )} • n = ${fitP.n}`;
    } else {
      profitStats.textContent = "";
    }
  }

  qiltX.addEventListener("change", renderAll);
  qiltFit.addEventListener("change", renderAll);
  profitX.addEventListener("change", renderAll);
  profitY.addEventListener("change", renderAll);
  profitFit.addEventListener("change", renderAll);
